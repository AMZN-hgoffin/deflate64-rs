...
fn decompress_with_checkpoints(
    input: &mut impl Read,
    output: &mut impl Write,
    checkpoint_file: &mut impl Write,
) -> std::io::Result<()> {
    ...
    loop {
        ...
        // Periodically save checkpoint (e.g., every 100MB)
        if bytes_written % (100 * 1024 * 1024) < 8192 {
            if let Some((data, _positions)) = inflater.checkpoint() {
                checkpoint_file.write_all(&data)?;
            }
        }
    }
    Ok(())
...
fn resume_from_checkpoint(
    ...
    let positions = match inflater.restore_from_checkpoint(checkpoint_data) {
        Some(p) => p,
        None => return Ok(false), // Invalid or incompatible checkpoint
    };
    ...
    // Continue normal decompression loop
    let mut input_buf = [0u8; 8192];
    let mut output_buf = [0u8; 8192];
    loop {
        let n = input.read(&mut input_buf)?;
